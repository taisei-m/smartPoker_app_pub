...
<v-ons-card>
  <div>
    ...
    <div id='stars2'></div>
    <div id='stars3'></div>
    <h1 class="box17 animated heartBeat slower">WINNER:{{ranking[0].name}}</h1>

    <ul v-for ="(item,index) of ranking" v-bind:key="index" class="ranking">
      <ol>{{index+1}}位:{{item.name}}</ol>
    </ul>
    <div class="glitch" data-text="悪魔的だ">悪魔的だ</div>
  </div>
  </v-ons-card>
  <div>
   <a @click="submit()" class="btn">
    もう一度
  </a>
  </div>
     </div>
...
export default {
  data() {
    return {
      ranking: this.$store.state.ranking,
    };
  },
  created() {
  },
...
.ranking ol:first-child::after,
.ranking ol:nth-child(2)::after,
.ranking ol:nth-child(3)::after{
    position: absolute;
    top: 9px;
    left: 0;
    width: 0;
